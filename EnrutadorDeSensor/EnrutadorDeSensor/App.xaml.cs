...
namespace EnrutadorDeSensor
{
    ...
    sealed partial class App : Application
    {
        ...
        /// </summary>
        /// 
        //En la clase app.xaml, permite crear una base de datos. En el constructor verificamos si existe la base de datos y si no la creamos. Dado que si no existe ningún archivo, recibirá una excepción.
        public static string DB_PATH = Path.Combine(Path.Combine(ApplicationData.Current.LocalFolder.Path, "LecturasManager.sqlite"));//DataBase Name
        public App()
        {
            this.InitializeComponent();
            this.Suspending += OnSuspending;

            if (!CheckFileExists("LecturasManager.sqlite").Result)
            {
                using (var db = new SQLiteConnection(new SQLitePlatformWinRT(), DB_PATH))
                {
                    db.CreateTable<Lectura>();
                }
            }
        }
